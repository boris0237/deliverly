...
export async function GET(request: Request) {
  try {
    ...
    const pageSize = Math.min(100, Math.max(1, Number(searchParams.get('pageSize') || '9') || 9));
    const search = (searchParams.get('search') || '').trim();

    const query: Record<string, unknown> = { companyId: company.id, isActive: true };
    if (search) {
    ...
    const pricing = buildPricingContext(company);
...
export async function POST(request: Request) {
  try {
    ...
    const now = new Date();
    const partnerId = randomToken(12);
    ...
    return NextResponse.json(
      {
        ...
        partner: mapPartner(created.toObject()),
      },
      { status: 201 }
    );
  } catch (error) {
